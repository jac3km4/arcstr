...
compile_error!(
    ...
    `rcstr` crate so we can talk about your use case if this is \
    important to you."
);

/// A low-cost string type representing a view into an [`RcStr`].
///
/// Conceptually this is `(RcStr, Range<usize>)` with ergonomic helpers. In
...
/// let parent = RcStr::from("foo   bar");
/// // The main way to create a Substr is with `RcStr::substr`.
...
/// // the functions with the same name on `RcStr`.
...
#[derive(Clone)]
#[repr(C)] // We mentioned RcStr being good at FFI at some point so why not
pub struct Substr(RcStr, Idx, Idx);
...
fn index_overflow(i: usize) -> ! {
    panic!("The index {} is too large for rcstr::Substr (enable the `substr-usize-indices` feature in `rcstr` if you need this)", i);
}
#[cold]
#[inline(never)]
fn bad_substr_idx(s: &RcStr, i: usize, e: usize) -> ! {
    ...
    unreachable!(
        "[rcstr bug]: should have failed one of the above tests: \
...
impl Substr {
    ...
    pub const fn new() -> Self {
        Substr(RcStr::new(), 0, 0)
    }

    /// Construct a Substr over the entire RcStr.
    ///
    ...
    /// # Examples
    /// ```
    /// # use rcstr::{Substr, RcStr};
    /// let s = Substr::full(RcStr::from("foo"));
    /// assert_eq!(s, "foo");
    /// assert_eq!(s.range(), 0..3);
    /// ```
    #[inline]
    pub fn full(a: RcStr) -> Self {
        let l = to_idx(a.len());
        Substr(a, 0, l)
    }

    #[inline]
    pub(crate) fn from_parts(a: &RcStr, range: impl RangeBounds<usize>) -> Self {
        use core::ops::Bound;
        ...
        Self(RcStr::clone(a), to_idx(begin), to_idx(end))
    }
    ...
    pub fn substr(&self, range: impl RangeBounds<usize>) -> Self {
        ...
        Self(RcStr::clone(&self.0), new_begin as Idx, new_end as Idx)
    }
    ...
    /// ```
    /// # use rcstr::Substr;
    /// let s: Substr = rcstr::literal!("foobar").substr(3..);
    ...
    /// ```
    /// # use rcstr::{RcStr, Substr};
    /// let a: Substr = RcStr::from("foo").substr(1..);
    /// assert_eq!(a.len(), 2);
    /// ```
    #[inline]
    ...
    /// ```
    /// # use rcstr::Substr;
    /// assert!(rcstr::literal!("abc").substr(3..).is_empty());
    /// assert!(!rcstr::literal!("abc").substr(2..).is_empty());
    ...
    /// ```
    /// # use rcstr::Substr;
    /// let s: Substr = rcstr::literal!("12345").substr(1..4);
    ...
    /// Unchecked function to cunstruct a [`Substr`] from an [`RcStr`] and a
    /// byte range. Direct usage of this function is largely discouraged in
    /// favor of [`RcStr::substr`][crate::RcStr::substr], or the
    ...
    /// usage).
    ///
    /// This is unsafe because currently `RcStr` cannot provide a `&str` in a
    ...
    /// ```
    /// use rcstr::{RcStr, Substr};
    /// const FOOBAR: RcStr = rcstr::literal!("foobar");
    ...
    /// compilation failure.
    #[inline]
    pub const unsafe fn from_parts_unchecked(s: RcStr, range: Range<usize>) -> Self {
    ...
    /// [`RcStr::ptr_eq`], which can have surprising/nondeterministic results
    /// when used on `const` `RcStr`s. It is guaranteed that `Substr::clone()`s
    ...
    pub fn shallow_eq(this: &Self, o: &Self) -> bool {
        RcStr::ptr_eq(&this.0, &o.0) && (this.1 == o.1) && (this.2 == o.2)
    }

    /// Returns the RcStr this is a substring of.
    ///
    /// Note that the exact pointer value of this can be somewhat
    /// nondeterministic when used with `const` `RcStr`s. For example
    ///
    /// ```rust,ignore
    /// const FOO: RcStr = rcstr::literal!("foo");
    /// // This is non-deterministic, as all references to a given
    /// // const are not required to point to the same value.
    /// RcStr::ptr_eq(FOO.substr(..).parent(), &FOO);
    /// ```
    ///
    /// # Examples
    ///
    /// ```
    /// # use rcstr::RcStr;
    /// let parent = RcStr::from("abc def");
    /// let child = parent.substr(2..5);
    /// assert!(RcStr::ptr_eq(&parent, child.parent()));
    ///
    /// let child = parent.substr(..);
    /// assert_eq!(child.range(), 0..7);
    /// ```
    #[inline]
    pub fn parent(&self) -> &RcStr {
        &self.0
    }
    ...
    pub fn substr_from(&self, substr: &str) -> Substr {
        ...
        // provide better error message like we do for RcStr
...
impl From<RcStr> for Substr {
    #[inline]
    fn from(a: RcStr) -> Self {
        Self::full(a)
    }
}
...
impl PartialEq<RcStr> for Substr {
    #[inline]
    fn eq(&self, o: &RcStr) -> bool {
        (RcStr::ptr_eq(&self.0, o) && (self.1 == 0) && (self.2 as usize == o.len()))
            || PartialEq::eq(self.as_str(), o.as_str())
    }
    #[inline]
    fn ne(&self, o: &RcStr) -> bool {
        (!RcStr::ptr_eq(&self.0, o) || (self.1 != 0) || (self.2 as usize != o.len()))
            && PartialEq::ne(self.as_str(), o.as_str())
    }
}
impl PartialEq<Substr> for RcStr {
    #[inline]
...
impl<'a> From<Substr> for alloc::borrow::Cow<'a, str> {
    ...
    fn from(s: Substr) -> Self {
        if let Some(st) = RcStr::as_static(&s.0) {
...
mod test {
    ...
    fn test_from_parts_unchecked_err() {
        ...
        // RcStr that's big enough, and I'm the author so I know it's fine
