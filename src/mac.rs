/// Create a const [`RcStr`](crate::RcStr) from a string literal. The
/// resulting `RcStr` require no heap allocation, can be freely cloned and used
/// interchangeably with `RcStr`s from the heap, and are effectively "free".
///
...
/// # use rcstr::RcStr;
/// // Works in const:
/// const MY_ARCSTR: RcStr = rcstr::literal!("testing testing");
/// assert_eq!(MY_ARCSTR, "testing testing");
///
/// // Or, just in normal expressions.
/// assert_eq!("Wow!", rcstr::literal!("Wow!"));
/// ```
///
...
/// ```rust,ignore
/// use rcstr::RcStr;
/// const VERY_IMPORTANT_FILE: RcStr =
///     rcstr::literal!(include_str!("./very-important.txt"));
/// ```
#[macro_export]
macro_rules! literal {
    ($text:expr $(,)?) => {{
        ...
        const __TEXT: &$crate::_private::str = $text;
        {
            ...
            const S: $crate::RcStr = unsafe { $crate::RcStr::_private_new_from_static_data(SI) };
            S
        }
    }};
}

/// Conceptually equivalent to `RcStr::from(format!("...", args...))`.
///
...
///
/// ```
/// let rcstr = rcstr::format!("testing {}", 123);
/// assert_eq!(rcstr, "testing 123");
/// ```
#[macro_export]
macro_rules! format {
    ($($toks:tt)*) => {
        $crate::RcStr::from($crate::alloc::fmt::format($crate::core::format_args!($($toks)*)))
    };
}
...
/// `const` [`RcStr`](crate::RcStr) made using [rcstr::literal!](crate::literal).
///
/// As with `rcstr::literal`, these require no heap allocation, can be freely
/// cloned and used interchangeably with `RcStr`s from the heap, and are
/// effectively "free".
///
/// The main use case here is in applications where `Substr` is a much more
/// common string type than `RcStr`.
///
/// # Examples
///
/// ```
/// use rcstr::{Substr, literal_substr};
/// // Works in const:
...
macro_rules! literal_substr {
    ($text:expr $(,)?) => {{
        ...
        {
            const PARENT: $crate::RcStr = $crate::literal!(__S);
